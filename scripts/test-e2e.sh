...
require_env() {
    if [ -z "${!1:-}" ]; then
        err "Required env var $1 is not set (put it in .env.test-e2e or export it)."
        exit 1
    fi
}
...
This file was generated by scripts/test-e2e.sh at $(date -u +"%Y-%m-%dT%H:%M:%SZ").
...
EOF
    ...
    PR_URL=$(GH_TOKEN="$GH_TEST_TOKEN" gh pr create \
        ...
        --body "Automated PR opened by scripts/test-e2e.sh — safe to close.")
